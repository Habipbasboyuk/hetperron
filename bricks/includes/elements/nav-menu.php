...
class Element_Nav_Menu extends Element {
	...
	public function set_controls() {
		...
		$this->controls['mobileMenuCustomBreakpoint'] = [
			'group'    => 'mobile-menu',
	...
	public function render() {
		...
		if ( $show_menu_toggle_at !== 'always' ) {
			...
			if ( bricks_is_builder() || bricks_is_builder_call() ) {
				if ( $show_menu_toggle_at === 'custom' && ! empty( $settings['mobileMenuCustomBreakpoint'] ) ) {
	...
	public function walker_nav_menu_start_el( $output, $item, $depth, $args ) {
		...
		// Default toggle SVG
		if ( ! $icon ) {
			$icon = '<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12" fill="none" class="menu-item-icon"><path d="M1.50002 4L6.00002 8L10.5 4" stroke-width="1.5" stroke="currentcolor"></path></svg>';
		}
